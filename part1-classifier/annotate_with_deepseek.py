"""
Скрипт для автоматической разметки примеров 1-45 с использованием Deepseek
Работает с любой LLM: Claude, Deepseek, OpenAI
"""
...
def main():
    ...
    print("1. Deepseek (deepseek-chat) - дешевый и быстрый ")
    print("2. Claude (claude-sonnet-4-5) - лучшая математика")
    print("3. OpenAI (gpt-4o)")

    provider_choice = input("\nВыберите провайдер (1-3) или нажмите Enter для Deepseek: ").strip()

    # Конфигурация провайдеров: (provider, api_key, base_url)
    provider_map = {
        '1': ('deepseek', os.environ.get('DEEPSEEK_API_KEY'), 'https://api.artemox.com/v1'),
        '2': ('claude', os.environ.get('ANTHROPIC_API_KEY'), None),
        '3': ('openai', os.environ.get('OPENAI_API_KEY'), None),
        '': ('deepseek', os.environ.get('DEEPSEEK_API_KEY'), 'https://api.artemox.com/v1')  # по умолчанию
    }

    if provider_choice not in provider_map:
        print("[ERROR] Неверный выбор, используем Deepseek")
        provider_choice = ''

    provider, api_key, base_url = provider_map[provider_choice]

    if not api_key:
        print(f"[ERROR] API ключ для {provider} не найден!")
        return
    ...
    print(f"\n Инициализация {provider.upper()}...")
    if base_url:
        print(f"   Используется прокси: {base_url}")
    classifier = UniversalMathErrorClassifier(provider=provider, api_key=api_key, base_url=base_url)
    ...
    if len(labeled_results) > 0:
        ...
        metrics = None
        print("\n⚠️  Нет примеров с ground truth для расчета метрик")

    # Сохранение
    output_dir = os.path.dirname(os.path.abspath(__file__))
    save_results(results, metrics, output_dir, provider)
